...
class BounceCards {
  constructor(container, options = {}) {
    ...
    this.options = {
      ...
      cardWidth: options.cardWidth || 200,
      cardHeight: options.cardHeight || 280,
  ...
  init() {
    ...
    this.container.style.overflow = 'visible'; // Ensure no clipping
    ...
    this.options.images.forEach((src, idx) => {
      ...
      card.style.marginTop = -(this.options.cardHeight / 2) + 'px';
      card.style.marginLeft = -(this.options.cardWidth / 2) + 'px';
  ...
  onMouseEnter(hoveredIdx) {
    ...
    this.cards.forEach((card, idx) => {
      ...
      if (idx === hoveredIdx) {
        ...
        // Push siblings away
        const offsetX = idx < hoveredIdx ? -200 : 200;
